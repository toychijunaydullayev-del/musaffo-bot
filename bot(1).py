...
# ─── SUV HAQIDA BO'LIMI ───────────────────────────────────────────────────────
# Rasmlar URL linklari (ibb.co dan)
WATER_PHOTOS = [
...
ABOUT_TEXT = (
    "💧 *Musaffo Chashma Tomchisi* — tabiiy mineral suv!\n\n"
    "Tog' manbalaridan kelib chiqqan, organizm uchun foydali "
    "minerallarga boy. Toza, shaffof va yangi.\n\n"
    "✅ Tabiiy manba\n"
    "✅ Minerallarga boy\n"
    "✅ Sog'lik uchun foydali\n\n"
    "🚰 _Toza suv — sog'lom hayot garovi!_"
)
...
def water_info_keyboard():
    """Suv haqida bo'limidagi qo'shimcha tugmalar"""
    return ReplyKeyboardMarkup(
        [
...
async def show_water_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Rasmlar galereya + qisqa ma'lumot yuboradi"""
    # 1) Rasmlar guruhini galereya sifatida yuborish
...
async def show_filter_details(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Filtrlar haqida batafsil ma'lumot"""
    await update.message.reply_text(
        WATER_FILTERS,
        parse_mode="Markdown",
        reply_markup=water_info_keyboard(),
